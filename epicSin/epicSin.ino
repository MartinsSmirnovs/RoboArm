...
int oldPosit = 150;//current motor position
int newPosit = 25; //new motor position

int delayTime = 25;//motor speed
...
void loop() {
  ...
  if (difference <= maxStep) {
    maxStep = difference / 2;
    if (maxStep == 0)
      maxStep = 1;
  }

  servo.write(oldPosit);
  delay(1000);
